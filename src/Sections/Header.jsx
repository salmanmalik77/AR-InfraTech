...
const Header = () => {
  ...
  return (
    <nav
      className="w-full flex bg-white justify-between items--center gap-1 lg:px-16
                     px-6 py-4 sticky top-0 z-50"
    >
      <h1 className="text-black md:text-4xl text-3xl font-bold font-rubik">
        AR
        <span className="text-yellow-500 italic">InfraTech</span>
      </h1>
        ...
        {navItems.map(({ link, path }) => (
          <Link
            ...
            className="text-black uppercase font-bold cursor-pointer 
                        py-3 rounded-full hover:bg-yellow-500 hover:text-black text-[15px]"
            to={path}
            spy={true}
            offset={-100}
            smooth={true}
